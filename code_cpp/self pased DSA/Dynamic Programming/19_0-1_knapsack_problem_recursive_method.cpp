...
// revision special
int solve(int Weight, int wt[], int val[], int n)
{
    if(n==0 || Weight==0) return 0;

    if(wt[n-1] > Weight){
        return solve(Weight, wt, val, n-1);
    }
    else{
        int include_kro = val[n-1] + solve(Weight-wt[n-1], wt, val, n-1);
        int skip_kro =  solve(Weight, wt, val, n-1);
        return max(include_kro, skip_kro);
    } 
...
int main(){
    ...
    cout<<"maximum values collected in knapsack: "<<solve(weight, w, v, n)<<endl;
