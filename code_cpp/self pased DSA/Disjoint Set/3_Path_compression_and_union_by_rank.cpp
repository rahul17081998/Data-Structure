/*
Union By Rank and Path Compression operations

Avg.TC=O(1): on an average it take constant time : proof--> Corman book
*/
...
int find(int x)
{
    if(parent[x]==x)
        return x;
    parent[x] = find(parent[x]); // recursively find parent
    return parent[x];
}
