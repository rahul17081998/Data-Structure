...
// Patience Sorting is a powerful technique that can transfrom your solutions to 
// Longest Increasing Subsequence type Dynamic Programming problems from O(n^2) toO(nlogn) complexity. 
// It will allow you to solve hard DP problems.
int lengthOfLIS(vector<int>& nums) 
{
    ...
    for(int i=1; i<nums.size(); i++)
    {
        if(nums[i] > temp.back())
            ...
            // subtracting temp.beging for integer value from iterator
...
int main(){
    ...
    nums.push_back(8);
    nums.push_back(10);
    cout<<"Longest common subsequence length: "<<lengthOfLIS(nums)<<endl;
    return 0;
}