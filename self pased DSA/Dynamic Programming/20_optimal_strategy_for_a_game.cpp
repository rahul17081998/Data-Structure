...
int solve(int arr[], int s, int e)
{
    ...
    // when you chose staring element(arr[s]) then:-
    ...
    return max( arr[s] + min(solve(arr, s+2, e), solve(arr, s+1, e-1)), // you chose starting elelemt
                arr[e] + min(solve(arr, s+1, e-1), solve(arr, s, e-2))  // you chose ending element
                );
}
// method 3
